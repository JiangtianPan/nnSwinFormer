...
from networks.robust_nnswinformer import build_robust_nnswinformer as ViT_robust_seg
...
if args.dataset == "Synapse":
    args.volume_path = args.root_path
    # print(os.path.join(args.volume_path, "test_vol_h5"))
...
if __name__ == "__main__":
    ...
    dataset_config = {
        args.dataset: {
            'root_path': args.root_path + '/test_vol_h5',
    ...
    net = ViT_robust_seg(config,
        img_size=args.img_size, num_classes=args.num_classes,
        use_mc_dropout=True,      # 如果你想做不确定性估计
        mc_dropout_p=0.2          # 和实验计划里的 MC Dropout 参数对齐
        ).cuda()

    snapshot = os.path.join(args.output_dir, 'best_10noise_robust_model.pth')
    if not os.path.exists(snapshot):
        snapshot = snapshot.replace('best_10noise_robust_model', 'epoch_' + str(args.max_epochs - 1))
