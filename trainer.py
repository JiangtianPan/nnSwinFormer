...
def trainer_synapse(args, model, snapshot_path):
    ...
    random_generator = RandomGenerator(output_size=[args.img_size, args.img_size], noise_config=args.noise_config)
    # db_train = Synapse_dataset(base_dir=args.root_path, list_dir=args.list_dir, split="train",
    #                            transform=transforms.Compose(
    #                                [RandomGenerator(output_size=[args.img_size, args.img_size])])
    #                                , noise_config=args.noise_config)
    db_train = Synapse_dataset(base_dir=args.root_path, list_dir=args.list_dir, split="train",
                               transform=transforms.Compose([random_generator]),
                               noise_config=args.noise_config)
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in tqdm(enumerate(train_loader), desc=f"Train: {epoch_num}", total=len(train_loader),
            ...
            # print(
            #     "DEBUG shapes:",
            #     "image", image_batch.shape,
            #     "outputs", outputs.shape,
            #     "label", label_batch.shape,
            #     "label min/max", label_batch.min().item(), label_batch.max().item()
            # )
            # ddd
        ...
        if (epoch_num + 1) % args.eval_interval == 0:
            ...
            with torch.no_grad():
                ...
                if batch_loss < best_loss:
                    save_mode_path = os.path.join(snapshot_path, 'best_20noise_robust_model.pth')
                    torch.save(model.state_dict(), save_mode_path)
                    best_loss = batch_loss
                else:
                    save_mode_path = os.path.join(snapshot_path, 'last_20noise_robust_model.pth')
                    torch.save(model.state_dict(), save_mode_path)
